...
//       computeRoot returns valid=false on a structurally invalid proof.
...
contract SubstrateMerkleProofAliasingTest is Test {
    ...
    // 5. The public computeRoot returns valid=false on a structurally invalid proof (this is the
    //    entry point used by Verification.sol for parachain-head proofs).
    function test_computeRoot_invalidReturnsFalse() public {
        uint256 n = 600;
        bytes32[] memory leaves = MerkleLibSubstrate.genLeaves(n);
        (bytes32[][] memory L, bytes32 root) = MerkleLibSubstrate.buildLevels(leaves);
        ...
        // A proof too short for index 0 is structurally invalid -> valid=false.
        (bool valid,) = c.computeRoot(leaves[0], 0, n, shortProof);
        assertFalse(valid, "structurally invalid proof must return valid=false");

        // The canonical proof for index 0 is valid and reconstructs the real root.
        bytes32[] memory good = MerkleLibSubstrate.proofFromLevels(L, 0);
        (bool ok, bytes32 got) = c.computeRoot(leaves[0], 0, n, good);
        assertTrue(ok, "canonical proof must be valid");
        assertEq(got, root, "valid proof reconstructs the real root");
    }
}
...
contract BeefyClientForgeRejectionTest is Test {
    ...
    uint128 constant N = 600; // validator set size
    ...
    function _commit() internal view returns (BeefyClient.Commitment memory c) {
        ...
        pl[0] = BeefyClient.PayloadItem({payloadID: hex"6d68", data: abi.encodePacked(forged)});
        c = BeefyClient.Commitment({
            blockNumber: blockNum, validatorSetID: setId, payload: pl
        });
    }
        ...
        )
    {
        ...
        bc = new BeefyClientMock(
            ...
            FS_SIGS,
            0,
            BeefyClient.ValidatorSet({id: setId, length: N, root: vroot}),
            BeefyClient.ValidatorSet({id: setId + 1, length: N, root: vroot})
        );

        forged = keccak256("ATTACKER-FORGED-MMR-ROOT");
        blockNum = uint32(bc.latestBeefyBlock() + 1);
...
contract FiatShamirForgeRejectionTest is Test {
    ...
    function _commit(uint32 bn) internal view returns (BeefyClient.Commitment memory c) {
        BeefyClient.PayloadItem[] memory pl = new BeefyClient.PayloadItem[](1);
        pl[0] = BeefyClient.PayloadItem({payloadID: hex"6d68", data: abi.encodePacked(forged)});
        c = BeefyClient.Commitment({
            blockNumber: bn, validatorSetID: SET_ID, payload: pl
        });
    }
    ...
    function test_fix_fiatShamirForgeRejected() public {
        ...
        bc = new BeefyClientMock(
            ...
            FS_SIGS,
            0,
            BeefyClient.ValidatorSet({id: SET_ID, length: N, root: vRoot}),
            BeefyClient.ValidatorSet({id: SET_ID + 1, length: N, root: vRoot})
        );
