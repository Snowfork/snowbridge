...
contract ForkUpgradeTest is Test {
    ...
    function checkLegacyToken() public {
        TokenInfo memory weth =
            Gateway202411(GATEWAY_PROXY).tokenInfo(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2);
        assertEq(weth.isRegistered, true);
        assertEq(weth.foreignID, bytes32(""));
        TokenInfo memory myth =
            Gateway202411(GATEWAY_PROXY).tokenInfo(0xBA41Ddf06B7fFD89D1267b5A93BFeF2424eb2003);
    ...
    function registerForeignToken() public {
        ...
        TokenInfo memory dot =
            Gateway202411(GATEWAY_PROXY).tokenInfo(0x70D9d338A6b17957B16836a90192BD8CDAe0b53d);
