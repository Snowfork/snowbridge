...
library SubstrateTypes {
    ...
    function EncodedValidatorsData(bytes32[] calldata validatorsKeys) internal view returns (bytes memory) {
        uint16 validatorsKeysLength = validatorsKeys.length;
        if (validatorsKeysLength > 1000) {
            revert SubstrateTypes__UnsupportedValidatorsLength();
        }

         bytes memory flattenedKeys = new bytes(validatorsKeys.length * 32);
    for (uint i = 0; i < validatorsKeys.length; i++) {
        bytes32 key = validatorsKeys[i];
        assembly {
            mstore(add(add(flattenedKeys, 32), mul(i, 32)), key)
        }
    }
        return bytes.concat(
            bytes4(0x70150038),
            bytes2(0x00),
            ScaleCodec.encodeU16(validatorsKeysLength),
