...
library Validators {
    ...
    uint8 private constant BYTES_LENGTH_PER_VALIDATOR = 32 * 2;

    function encodeValidatorsData(bytes calldata validatorsKeys, ParaID dest) internal returns (Ticket memory ticket) {
        if (validatorsKeys.length % BYTES_LENGTH_PER_VALIDATOR != 0) {
            revert Validators__UnsupportedValidatorsLength();
        }
        uint256 validatorsKeysLength = validatorsKeys.length / BYTES_LENGTH_PER_VALIDATOR;
