...
library SubstrateTypes {
    ...
    function EncodedValidatorsData(bytes32[] calldata validatorsKeys) internal pure returns (bytes memory) {
        ...
        return bytes.concat(
            bytes4(0x70150038), bytes2(0x00), ScaleCodec.encodeU16(uint16(validatorsKeysLength)), flattenedKeys
        );
    }
}
