...
fn initial_sync_from_file<T: crate::Config>(name: &str) -> InitialSync<T::MaxSyncCommitteeSize, T::MaxProofBranchSize> {
	let filepath = fixture_path(name);
	serde_json::from_reader(File::open(&filepath).unwrap()).unwrap()
}

fn sync_committee_update_from_file<T: crate::Config>(name: &str) -> SyncCommitteePeriodUpdate<T::MaxSignatureSize, T::MaxProofBranchSize, T::MaxSyncCommitteeSize> {
	let filepath = fixture_path(name);
	serde_json::from_reader(File::open(&filepath).unwrap()).unwrap()
}

fn finalized_header_update_from_file<T: crate::Config>(name: &str) -> FinalizedHeaderUpdate<T::MaxSignatureSize, T::MaxProofBranchSize, T::MaxSyncCommitteeSize> {
	let filepath = fixture_path(name);
	serde_json::from_reader(File::open(&filepath).unwrap()).unwrap()
}

fn block_update_from_file<T: crate::Config>(name: &str) -> BlockUpdate<
	T::MaxFeeRecipientSize, 
	T::MaxLogsBloomSize, 
	T::MaxExtraDataSize, 
	T::MaxDepositDataSize, 
...
fn attester_slashing_from_file<T: crate::Config>(name: &str) -> AttesterSlashing<T::MaxValidatorsPerCommittee, T::MaxSignatureSize> {
	let filepath = fixture_path(name);
	serde_json::from_reader(File::open(&filepath).unwrap()).unwrap()
}
...
pub fn get_initial_sync<T: crate::Config>() -> InitialSync<T::MaxSyncCommitteeSize, T::MaxProofBranchSize> {
	initial_sync_from_file::<T>(&add_file_prefix("initial_sync.json"))
}

pub fn get_committee_sync_period_update<T: crate::Config>() -> SyncCommitteePeriodUpdate<T::MaxSignatureSize, T::MaxProofBranchSize, T::MaxSyncCommitteeSize> {
...
pub fn get_finalized_header_update<T: crate::Config>() -> FinalizedHeaderUpdate<T::MaxSignatureSize, T::MaxProofBranchSize, T::MaxSyncCommitteeSize> {
...
pub fn get_attester_slashing<T: crate::Config>() -> AttesterSlashing<T::MaxValidatorsPerCommittee, T::MaxSignatureSize> {
