...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn submit(who: &T::AccountId, target: H160, payload: &[u8]) -> DispatchResult {
			...
			<NextId<T>>::try_mutate(|id| -> DispatchResult {
				if let Some(v) = id.checked_add(1) {
					*id = v;
				} else {
					return Err(Error::<T>::Overflow.into());
				}

				<MessageQueue<T>>::append(Message { id: *id, target, payload: payload.to_vec() });
				Self::deposit_event(Event::MessageAccepted(*id));
				Ok(())
			})
		}
		...
		fn commit() -> Weight {
			...
			let commitment_hash = Self::make_commitment_hash(next_nonce, &messages);
		...
		fn make_commitment_hash(nonce: u64, messages: &[Message]) -> H256 {
