...
benchmarks! {
	lock_basic_channel {
		...
		<T as DotAppConfig>::Currency::make_free_balance_be(&caller, balance);
	...
	verify {
		...
		assert_eq!(<T as DotAppConfig>::Currency::free_balance(&lock_account), amount);
	}
	...
	unlock {
		...
		// Create DOT account for caller
		<T as DotAppConfig>::Currency::make_free_balance_be(&recipient, 0u32.into());
	...
	verify {
		assert!(!balance.is_zero() && !amount.is_zero());
		assert_eq!(<T as DotAppConfig>::Currency::free_balance(&lock_account), balance - amount);
		assert_eq!(<T as DotAppConfig>::Currency::free_balance(&recipient), amount);
	}
