...
contract IncentivizedInboundChannel is AccessControl {
    ...
    // Governance contracts will administer using these roles
    bytes32 public constant CONFIG_UPDATE_ROLE = keccak256("CONFIG_UPDATE_ROLE");
    bytes32 public constant BEEFY_UPGRADE_ROLE = keccak256("BEEFY_UPGRADE_ROLE");

    RewardSource private rewardSource;

    ParachainClient public parachainClient;

    event Upgraded(address upgrader, ParachainClient parachainClient);
    ...
    function upgrade(ParachainClient _parachainClient) external onlyRole(BEEFY_UPGRADE_ROLE) {
