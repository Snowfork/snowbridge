  ...
  describe("send", function () {
    it("should send messages out with the correct event and fields", async function () {
      ...
      await expect(channel.connect(app).submit(
        ...
        testPayload,
        0
      )).to.emit(channel, "Message").withArgs(app.address, user.address, 1, testPayload);
    });
    ...
    it("should increment nonces correctly", async function () {
      ...
      await expect(channel.connect(app).submit(
        ...
        testPayload,
        0
      )).to.emit(channel, "Message").withArgs(app.address, user.address, 2, testPayload);

      await expect(channel.connect(app).submit(
        user.address,
        testPayload,
        0
      )).to.emit(channel, "Message").withArgs(app.address, user.address, 3, testPayload);
    });
  });
});
