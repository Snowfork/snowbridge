...
import {
    buildAssetHubPNAReceivedXcmForKusama,
    sendMessageXCM,
} from "../../xcmbuilders/toKusama/pnaToKusamaAH"
...
export class PNAToKusamaAH<T extends EthereumProviderTypes> implements TransferInterface<T> {
    ...
    async fee(
        ...
        let assetHubXcm = buildAssetHubPNAReceivedXcmForKusama(
            assetHub.registry,
            registry.ethChainId,
        ...
        // Kusama delivery fee (via bridge)
    ...
    async validate(transfer: Transfer<T>): Promise<ValidatedTransfer<T>> {
        ...
        if (logs.length === 0) {
            const [estimatedGas, feeData] = await Promise.all([
                context.ethereumProvider.estimateGas(ethereum, transfer.tx),
        ...
        if (kusamaAHParachain?.features?.hasDryRunApi) {
            ...
            let tokenLocation = ahAssetMeta?.location
            if (!tokenLocation) {
