...
type IGatewayProxy = {
    getFunction(name: "v2_dispatch"): {
        populateTransaction(
            commands: { kind: number; gas: bigint; payload: string }[],
            origin: string,
            nonce: bigint,
    ...
    interface: {
        parseLog(log: unknown): { name: string; args: any }
    }
}
...
export class Context<T extends EthereumProviderTypes> {
    ...
    #gatewayProxy?: T["Contract"] & IGatewayProxy
    ...
    gatewayProxy(): T["Contract"] & IGatewayProxy {
        ...
        this.#gatewayProxy = this.ethereumProvider.connectContract(
            this.environment.gatewayContract,
            IGATEWAY_V2_ABI as T["Abi"],
            this.ethereum(),
        ) as T["Contract"] & IGatewayProxy
        return this.#gatewayProxy
    }
