...
export const getSubstrateAccount = async (
	parachain: ApiPromise,
	beneficiaryHex: string,
) => {
	const account = (
		await parachain.query.system.account(beneficiaryHex)
	).toPrimitive() as {
		data: { free: string };
		consumers: number;
	};
...
export const validateSend = async (
	...
	if (destinationParaId === assetHubParaId) {
		if (destinationFee !== 0n)
			throw new Error("Asset Hub does not require a destination fee.");
		...
		if (context.hasParachain(destinationParaId)) {
			...
			beneficiaryAccountExists =
				options.ignoreExistentialDeposit ||
				BigInt(balance) > existentialDeposit;
	...
	if (!hasConsumers)
		errors.push({
			...
			message:
				"Benificiary is approaching the asset consumer limit. Transfer may fail.",
		});
	if (!hrmpChannelSetup)
		errors.push({
...
export const send = async (
	...
	const messageAccepted = events.find(
		(log) => log.name === "OutboundMessageAccepted",
	);
...
export const trackSendProgressPolling = async (
	...
	if (success.bridgeHub.beaconUpdate === undefined) {
		...
		let { found, lastScannedBlock } = await scanSubstrateEvents(
			...
			async (n, blockHash, ev) => {
				...
				if (
					bridgeHub.events.ethereumBeaconClient.BeaconHeaderImported.is(
						event.event,
					)
				) {
					...
					const ethBlockHash =
						slot.data.message.body.execution_payload?.block_hash;
					if (
		...
		console.log(
			`Included by light client in Bridge Hub block ${beaconUpdate?.createdAtHash}.`,
		);
	}

	if (success.bridgeHub.events === undefined) {
		console.log(
			`Waiting for messageId ${success.messageId} to be recieved on Bridge Hub.`,
		);
		...
		let { found, lastScannedBlock, events } = await scanSubstrateEvents(
			...
			async (n, blockHash, ev) => {
				...
				if (
					bridgeHub.events.ethereumInboundQueue.MessageReceived.is(
						event.event,
					) &&
					...
					success.bridgeHub.extrinsicNumber =
						event.phase.toPrimitive().applyExtrinsic;
				}
	...
	if (
		success.assetHub.events === undefined &&
		success.bridgeHub.extrinsicSuccess === true
	) {
		const issuedTo =
		...
		console.log(
			`Waiting for messageId ${success.messageId} to be recieved on Asset Hub.`,
		);
			...
			async (n, blockHash, ev) => {
				...
				if (
					...
					eventData[2].toString() ===
						success?.plan.success?.amount.toString() &&
	...
	if (
		...
		success.destinationParachain.messageQueueProcessedAt =
			messageQueueProcessedAt;
		console.log(
...
export async function* trackSendProgress(
	...
	if (success.bridgeHub.beaconUpdate === undefined) {
		...
		const lastBeaconUpdate = await lastValueFrom(
			bridgeHub.rx.query.ethereumBeaconClient.latestFinalizedBlockRoot().pipe(
				...
				takeWhile(
					({ blockNumber }) => ethereumBlockNumber > Number(blockNumber),
				),
				...
				filter((events) => {
					...
					for (const event of events_iter) {
						...
						if (
							...
							eventData.channelId.toLowerCase() ===
								success?.channelId.toLowerCase()
						) {
	...
	if (success.assetHub.events === undefined) {
		...
		const { allEvents: receivedEvents, extrinsicSuccess } =
