...
export type BridgeStatusInfo = {
    ...
    toPolkadot: {
        ...
        blockLatency: number
        latencySeconds: number
        latestEthereumBeaconSlotOnPolkadot: number
        previousEthereumBeaconSlotOnPolkadot: number
    }
}
...
export const bridgeStatusInfo = async (
    ...
    const latestBeefyBlock = Number(
        await context.ethereum.contracts.beefyClient.latestBeefyBlock({
            blockTag: latestEthereumBlock,
        })
    )
    const previousBeefyBlock = Number(
        await context.ethereum.contracts.beefyClient.latestBeefyBlock({
            blockTag:
                latestEthereumBlock > options.previousStateInBlock
                    ? latestEthereumBlock - options.previousStateInBlock
                    : 0,
        })
    )
    ...
    const previousBridgeHubBlock = await context.polkadot.api.bridgeHub.query.system.blockHash(
        latestBridgeHubBlock > options.previousStateInBlock
            ? latestBridgeHubBlock - options.previousStateInBlock
            : 0
    )
    ...
    return {
        ...
        toPolkadot: {
            ...
            latestEthereumBeaconSlotOnPolkadot: Number(latestBeaconSlot.data.message.slot),
            previousEthereumBeaconSlotOnPolkadot: Number(previousBeaconSlot.data.message.slot),
        },
    }
}
...
export const channelStatusInfo = async (
    context: Context,
    channelId: string,
    options = { previousStateInBlock: 50 }
