...
export async function createTransfer(
	...
	const totalFeeDot =
		fee.destinationDeliveryFeeDOT + fee.destinationExecutionFeeDOT;
	const tx = await con
...
export async function validateTransfer(
	...
	if (destinationParaId !== registry.assetHubParaId) {
		...
		if (!ahAssetMetadata.isSufficient && !dryRunAhSuccess) {
			const sovereignAccountId = paraIdToSovereignAccount(
				"sibl",
				destinationParaId,
			);
			...
			if (
				...
				if (!accountExists) {
					logs.push({
						...
						message:
							"Beneficiary account does not exist on the destination chain.",
					});
				}
			}
		}
		...
		if (accountMaxConumers) {
			logs.push({
				...
				reason: ValidationReason.MaxConsumersReached,
				message:
					"Beneficiary account has reached the max consumer limit on Asset Hub.",
			});
		}
		if (!accountExists) {
			logs.push({
	...
	const success =
		logs.find((l) => l.kind === ValidationKind.Error) === undefined;
...
function resolveInputs(
	...
	const tokenErcMetadata =
		registry.ethereumChains[registry.ethChainId.toString()].assets[
...
async function dryRunAssetHub(assetHub: ApiPromise, transfer: Transfer) {
	...
	const bridgeHubLocation = {
		v4: {
			parents: 1,
			interior: { x1: [{ parachain: registry.bridgeHubParaId }] },
		},
	};
	let xcm: any;
...
async function dryRunDestination(
	...
	const assetHubOrigin = {
		v4: {
			parents: 1,
			interior: { x1: [{ parachain: registry.assetHubParaId }] },
		},
	};
