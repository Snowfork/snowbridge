...
func (h *Header) SyncHeader(ctx context.Context, headerUpdate scale.HeaderUpdate, slotsLeft uint64) error {
	...
	log.WithFields(log.Fields{
		"slot":                 headerUpdate.Payload.AttestedHeader.Slot,
	...
	for currentSlot <= toSlot {
		...
		if h.syncer.IsStartOfEpoch(currentSlot) {
			...
			for _, header := range headersToSync {
				err := h.SyncHeader(ctx, header, toSlot-uint64(header.Payload.AttestedHeader.Slot))
				if err != nil {
		...
		headerUpdate.Payload.SignatureSlot = nextHeaderUpdate.Payload.AttestedHeader.Slot
		...
		currentSlot = uint64(nextHeaderUpdate.Payload.AttestedHeader.Slot)
	}
