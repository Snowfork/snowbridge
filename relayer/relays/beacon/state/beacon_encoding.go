...
// Hash: 75f4d4e618b8d10281d4888d8d3cfb8b7d3e24fa490961c9578cb9644ac4997d
// Version: 0.1.3-dev
package state
...
// MarshalSSZ ssz marshals the BlockRootsContainer object
func (b *BlockRootsContainer) MarshalSSZ() ([]byte, error) {
	return ssz.MarshalSSZ(b)
}

// MarshalSSZTo ssz marshals the BlockRootsContainer object to a target array
func (b *BlockRootsContainer) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf

	// Field (0) 'BlockRoots'
	if size := len(b.BlockRoots); size != 8192 {
		err = ssz.ErrVectorLengthFn("BlockRootsContainer.BlockRoots", size, 8192)
		return
	}
	for ii := 0; ii < 8192; ii++ {
		if size := len(b.BlockRoots[ii]); size != 32 {
			err = ssz.ErrBytesLengthFn("BlockRootsContainer.BlockRoots[ii]", size, 32)
			return
		}
		dst = append(dst, b.BlockRoots[ii]...)
	}
...
// UnmarshalSSZ ssz unmarshals the BlockRootsContainer object
func (b *BlockRootsContainer) UnmarshalSSZ(buf []byte) error {
	var err error
	size := uint64(len(buf))
	if size != 262144 {
		return ssz.ErrSize
...
// SizeSSZ returns the ssz encoded size in bytes for the BlockRootsContainer object
func (b *BlockRootsContainer) SizeSSZ() (size int) {
	size = 262144
	return
}

// HashTreeRoot ssz hashes the BlockRootsContainer object
func (b *BlockRootsContainer) HashTreeRoot() ([32]byte, error) {
	return ssz.HashWithDefaultHasher(b)
}

// HashTreeRootWith ssz hashes the BlockRootsContainer object with a hasher
func (b *BlockRootsContainer) HashTreeRootWith(hh ssz.HashWalker) (err error) {
	indx := hh.Index()
...
// GetTree ssz hashes the BlockRootsContainer object
func (b *BlockRootsContainer) GetTree() (*ssz.Node, error) {
	return ssz.ProofTree(b)
}
