...
func (wr *ParachainWriter) queryImportedHeaderExists(hash types.H256) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("storage query failed for key %s hash %s with error: %s", key.Hex(), hash.Hex(), err)
	}
	if !ok {
		return false, fmt.Errorf("storage query did not find header for key %s hash %s", key.Hex(), hash.Hex())
	}
