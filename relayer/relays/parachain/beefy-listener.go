...
func (li *BeefyListener) Start(ctx context.Context, eg *errgroup.Group) error {
	...
	accounts, err := li.config.getAccountIDs()
	if err != nil {
		return err
	}
	li.accountIDs = accounts
...
func (li *BeefyListener) discoverCatchupTasks(
	...
	basicNoncesForAccountIDs := make([]AccountNonces, len(li.accountIDs))
	for _, accountID := range li.accountIDs {
		...
		log.WithFields(log.Fields{
			"nonce":   ethBasicNonce,
			"account": types.HexEncodeToString(accountID[:]),
		...
		if err != nil {
			return nil, fmt.Errorf("create storage key for account '%v': %w", types.HexEncodeToString(accountID[:]), err)
		}
		...
		basicNoncesForAccountIDs = append(basicNoncesForAccountIDs, AccountNonces{
			accountID, uint64(paraBasicNonce), ethBasicNonce,
		})
	}
	...
	for _, nonceForAccountID := range basicNoncesForAccountIDs {
		if nonceForAccountID.paraBasicNonce > nonceForAccountID.ethBasicNonce {
			basicAccountIDsAndNoncesToFind = append(basicAccountIDsAndNoncesToFind, AccountNonceToFind{
				nonceForAccountID.accountID, nonceForAccountID.ethBasicNonce + 1,
			})
		}
	}
	...
	tasks, err := li.scanForCommitments(
		ctx,
		paraBlock,
		basicAccountIDsAndNoncesToFind,
...
func (li *BeefyListener) scanForCommitments(
	...
	basicAccountIDsAndNoncesToFind []AccountNonceToFind,
	...
	basicAccountIDsToCheck := make(map[types.AccountID]bool, len(basicAccountIDsAndNoncesToFind))
	for _, basicAccountIDAndNonce := range basicAccountIDsAndNoncesToFind {
		basicAccountIDsToCheck[basicAccountIDAndNonce.accountID] = true
	}
	...
	for (!scanBasicChannelDone || !scanIncentivizedChannelDone) && currentBlockNumber > 0 {
		...
		for _, digestItem := range digestItems {
			...
			if channelID.IsBasic && !scanBasicChannelDone {
				...
				for _, accountIDAndNonce := range basicAccountIDsAndNoncesToFind {
					_, shouldCheckAccountID := basicAccountIDsToCheck[accountIDAndNonce.accountID]
					if !shouldCheckAccountID {
						continue
					}
					...
					// blocks.
					if bundleNonce < accountIDAndNonce.nonceToFind {
						log.Debugf(
...
func bundleIndexForAccountID(bundles []BasicOutboundChannelMessageBundle, accountID *[32]byte) int {
	bundleIndex := -1
