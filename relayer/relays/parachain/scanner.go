...
func (s *Scanner) findTasks(
	...
	ethNonce, err := inboundQueueContract.Nonce(&options, s.config.SourceID)
	if err != nil {
		return nil, fmt.Errorf("fetch nonce from inbound queue contract for sourceID '%v': %w", s.config.SourceID, err)
	}
	...
	binary.LittleEndian.PutUint32(sourceIDBytes, s.config.SourceID)
	paraNonceKey, err := types.CreateStorageKey(s.paraConn.Metadata(), "OutboundQueue", "Nonce", sourceIDBytes, nil)
	if err != nil {
		return nil, fmt.Errorf("create storage key for parachain outbound queue nonce with sourceID '%v': %w", s.config.SourceID, err)
	}
	...
	if !(uint64(paraNonce) > ethNonce) {
		// TODO: compare to return nil, nil
		// tasks := make([]*Task, 0)
		// return tasks, nil
		return nil, nil
	}
