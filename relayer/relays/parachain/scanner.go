...
func (s *Scanner) findTasks(
	...
	ethNonce, err := inboundQueueContract.Nonce(&options, s.sourceID)
	if err != nil {
		return nil, fmt.Errorf("fetch nonce from inbound queue contract for sourceID '%v': %w", s.sourceID, err)
	}
	...
	binary.LittleEndian.PutUint32(sourceIDBytes, s.sourceID)
	paraNonceKey, err := types.CreateStorageKey(s.paraConn.Metadata(), "OutboundQueue", "Nonce", sourceIDBytes, nil)
	if err != nil {
		return nil, fmt.Errorf("create storage key for parachain outbound queue nonce with sourceID '%v': %w", s.sourceID, err)
	}
