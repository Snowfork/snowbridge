...
// CompleteSignatureCommitment is a paid mutator transaction binding the contract method 0x4b1abf58.
//
// Solidity: function completeSignatureCommitment(uint256 id, (bytes32,uint64,uint32) commitment, (bytes[],uint256[],address[],bytes32[][]) validatorProof, (uint8,uint32,bytes32,bytes32,uint64,uint32,bytes32) latestMMRLeaf, bytes32[] mmrProofItems) returns()
...
// VerifyNewestMMRLeaf is a paid mutator transaction binding the contract method 0xb1ceabc6.
//
// Solidity: function verifyNewestMMRLeaf((uint8,uint32,bytes32,bytes32,uint64,uint32,bytes32) leaf, bytes32[] proof, bytes32 root, uint64 length) returns()
