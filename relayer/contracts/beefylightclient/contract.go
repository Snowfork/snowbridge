...
type BeefyLightClientCommitment struct {
	Payload        []BeefyLightClientPayloadItem
	BlockNumber    uint32
	ValidatorSetId uint64
}

// BeefyLightClientPayloadItem is an auto generated low-level Go binding around an user-defined struct.
type BeefyLightClientPayloadItem struct {
	Id   [2]byte
	Data []byte
}
...
// CompleteSignatureCommitment is a paid mutator transaction binding the contract method 0x7ccaf090.
//
// Solidity: function completeSignatureCommitment(uint256 id, ((bytes2,bytes)[],uint32,uint64) commitment, (bytes[],uint256[],address[],bytes32[][]) validatorProof, (uint8,uint32,bytes32,uint64,uint32,bytes32,bytes32) latestMMRLeaf, (bytes32[],uint64) proof) returns()
