...
//
// Solidity: event Message(bytes dest, uint64 nonce, bytes payload)
func (_OutboundChannel *OutboundChannelFilterer) FilterMessage(opts *bind.FilterOpts) (*OutboundChannelMessageIterator, error) {

	logs, sub, err := _OutboundChannel.contract.FilterLogs(opts, "Message")
	if err != nil {
		return nil, err
	}
...
//
// Solidity: event Message(bytes dest, uint64 nonce, bytes payload)
func (_OutboundChannel *OutboundChannelFilterer) WatchMessage(opts *bind.WatchOpts, sink chan<- *OutboundChannelMessage) (event.Subscription, error) {

	logs, sub, err := _OutboundChannel.contract.WatchLogs(opts, "Message")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
