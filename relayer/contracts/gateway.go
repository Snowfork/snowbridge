...
//
// Solidity: event TokenSent(address indexed sender, address indexed token, uint32 indexed destinationChain, (uint8,bytes) destinationAddress, uint128 amount)
func (_Gateway *GatewayFilterer) FilterTokenSent(opts *bind.FilterOpts, sender []common.Address, token []common.Address, destinationChain []uint32) (*GatewayTokenSentIterator, error) {
	...
	logs, sub, err := _Gateway.contract.FilterLogs(opts, "TokenSent", senderRule, tokenRule, destinationChainRule)
	if err != nil {
		return nil, err
	}
...
//
// Solidity: event TokenSent(address indexed sender, address indexed token, uint32 indexed destinationChain, (uint8,bytes) destinationAddress, uint128 amount)
func (_Gateway *GatewayFilterer) WatchTokenSent(opts *bind.WatchOpts, sink chan<- *GatewayTokenSent, sender []common.Address, token []common.Address, destinationChain []uint32) (event.Subscription, error) {
	...
	logs, sub, err := _Gateway.contract.WatchLogs(opts, "TokenSent", senderRule, tokenRule, destinationChainRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
