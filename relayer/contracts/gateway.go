...
// GatewayTopicIterator is returned from FilterTopic and is used to iterate over the raw logs and unpacked data for Topic events raised by the Gateway contract.
type GatewayTopicIterator struct {
	Event *GatewayTopic // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *GatewayTopicIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *GatewayTopicIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *GatewayTopicIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// GatewayTopic represents a Topic event raised by the Gateway contract.
type GatewayTopic struct {
	TopicID [32]byte
	Raw     types.Log // Blockchain specific contextual infos
}

// FilterTopic is a free log retrieval operation binding the contract event 0xff076dd0e7b4feaccc2daf62ded04e4a813b1cd467b86b1a03eb8b0e5cb8763a.
//
// Solidity: event Topic(bytes32 topicID)
func (_Gateway *GatewayFilterer) FilterTopic(opts *bind.FilterOpts) (*GatewayTopicIterator, error) {

	logs, sub, err := _Gateway.contract.FilterLogs(opts, "Topic")
	if err != nil {
		return nil, err
	}
	return &GatewayTopicIterator{contract: _Gateway.contract, event: "Topic", logs: logs, sub: sub}, nil
}

// WatchTopic is a free log subscription operation binding the contract event 0xff076dd0e7b4feaccc2daf62ded04e4a813b1cd467b86b1a03eb8b0e5cb8763a.
//
// Solidity: event Topic(bytes32 topicID)
func (_Gateway *GatewayFilterer) WatchTopic(opts *bind.WatchOpts, sink chan<- *GatewayTopic) (event.Subscription, error) {

	logs, sub, err := _Gateway.contract.WatchLogs(opts, "Topic")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(GatewayTopic)
				if err := _Gateway.contract.UnpackLog(event, "Topic", log); err != nil {
					return err
				}
				event.Raw = log
...
// ParseTopic is a log parse operation binding the contract event 0xff076dd0e7b4feaccc2daf62ded04e4a813b1cd467b86b1a03eb8b0e5cb8763a.
//
// Solidity: event Topic(bytes32 topicID)
func (_Gateway *GatewayFilterer) ParseTopic(log types.Log) (*GatewayTopic, error) {
	event := new(GatewayTopic)
	if err := _Gateway.contract.UnpackLog(event, "Topic", log); err != nil {
		return nil, err
	}
