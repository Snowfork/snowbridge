...
type SimplifiedMMRProof struct {
	RestOfThePeaks           [][32]byte
	RightBaggedPeak          [32]byte
...
// Submit is a paid mutator transaction binding the contract method 0xd5121862.
//
// Solidity: function submit((address,uint64,uint256,bytes)[] _messages, (bytes,bytes,(uint256,uint256,bytes32[])) _parachainVerifyInput, (uint8,uint32,bytes32,uint64,uint32,bytes32) _beefyMMRLeafPartial, (bytes32[],bytes32,bytes32[],uint64) proof) returns()
