...
async fn do_something_through_sovereign() {
    ...
    let agent_address: Address = gateway.agent_of(ASSET_AGENT).await.unwrap();
    println!("{:?}", agent_address);

    let agent_encoded =
        (b"ethereum", 15u64, agent_address.as_fixed_bytes()).using_encoded(blake2_256);
    let agent_owner = AccountId32::from(agent_encoded);
    println!("{:?}", agent_owner.to_string());
