...
#[tokio::test]
async fn transfer_polkadot_token_v2() {
	...
	let weth_asset_location_copy: Location = Location {
		parents: 2,
		interior: Junctions::X2([
			GlobalConsensus(NetworkId::Ethereum { chain_id: ETHEREUM_CHAIN_ID }),
			AccountKey20 { network: None, key: *WETH_CONTRACT },
	...
	let remote_fee_asset_copy =
		Asset { id: AssetId(weth_asset_location_copy), fun: Fungible(remote_fee_amount) };
	...
	let xcms = VersionedXcm::V5(Xcm(vec![
		...
		InitiateTransfer {
			...
			remote_fees: Some(AssetTransferFilter::ReserveWithdraw(Definite(Assets(
				vec![remote_fee_asset_copy].into(),
			)))),
	...
	let token_transfer_call =
		TransactionApi.execute(xcms, Weight { ref_time: 8_000_000_000, proof_size: 80_000 });
